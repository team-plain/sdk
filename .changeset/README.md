...
When your change should ship to npm, run `pnpm changeset` at the repo root, pick **`@team-plain/sdk`**, and write a short summary. The codegen plugin is internal and is not offered in the wizard (`ignore` in `config.json`).
